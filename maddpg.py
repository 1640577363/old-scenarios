...
class MADDPG:
    ...
    def learn(self, memory, total_steps):
        # 方法判断缓冲区是否达到批处理大小（batch_size），避免在样本不足时更新网络
        if not memory.ready():
            # 如果缓冲区未准备好，返回 None 作为损失，表示没有进行学习
            return None, None
        ...
        for agent_idx, agent in enumerate(self.agents):
            ...
            step_critic_losses.append(critic_loss.item()) # 存储 Critic 损失值

            agent.critic.optimizer.zero_grad() # 清空梯度
            critic_loss.backward(retain_graph=True) # 反向传播，保留计算图 (重要，因为其他智能体可能还会用到它)
            agent.critic.optimizer.step() # 更新网络参数
            agent.critic.scheduler.step() # 更新学习率
            ...
            # 当前智能体的状态
            mu_states = T.tensor(actor_states[agent_idx], dtype=T.float).to(device)
            oa = old_actions.clone() # 克隆联合动作

            # 替换联合动作中当前智能体的部分为 Actor 网络预测的新动作
            start_idx = agent_idx * self.n_actions
            end_idx = start_idx + self.n_actions
            oa[:, start_idx:end_idx] = agent.actor.forward(mu_states)

            # 计算 Actor 损失：最大化 Critic 对修改后动作的评价（取负号）
            actor_loss = -T.mean(agent.critic.forward(states, oa).flatten())

            step_actor_losses.append(actor_loss.item()) # 存储 Actor 损失值

            agent.actor.optimizer.zero_grad()
            actor_loss.backward(retain_graph=True) # 反向传播，保留计算图
            agent.actor.optimizer.step()
            agent.actor.scheduler.step()

            # --- TensorBoard 日志（可选，如果你想保留）---
            # self.writer.add_scalar(f'Agent_{agent_idx}/Actor_Loss', actor_loss.item(), total_steps)
            # self.writer.add_scalar(f'Agent_{agent_idx}/Critic_Loss', critic_loss.item(), total_steps)
            # for name, param in agent.actor.named_parameters():
            #     if param.grad is not None:
        ...
        # --- 返回本次学习步骤的平均损失 ---
        # 计算当前学习步骤所有智能体的平均损失
        avg_actor_loss = np.mean(step_actor_losses)
        avg_critic_loss = np.mean(step_critic_losses)
